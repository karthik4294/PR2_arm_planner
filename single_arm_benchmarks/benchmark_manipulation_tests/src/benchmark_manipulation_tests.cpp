...
BenchmarkManipulationTests::BenchmarkManipulationTests() : ph_("~")
{
  ...
  sg_pub_ = nh_.advertise<visualization_msgs::MarkerArray>("sg_marker", 1000);
  path_marker_ = nh_.advertise<visualization_msgs::MarkerArray>("path_marker", 1000);
}
...
bool BenchmarkManipulationTests::getParams()
{
  ...
  ph_.param<std::string>("known_objects_filename",known_objects_filename_, "tabletop.env");
  ph_.param<std::string>("sbpl_planner_id",sbpl_planner_id_, "Hstar");
  ph_.param<std::string>("ompl_planner_id",ompl_planner_id_, "RRT");
  ph_.param<std::string>("eps_file",eps_file_, "eps_100");
  ph_.param<std::string>("timeout_file",timeout_file_, "20_timeout");
  ph_.param<std::string>("environment_name",environment_name_, "tabletop");
  ph_.param("exp_run_count",exp_run_count_, 20);
...
void BenchmarkManipulationTests::visualizePaths()
{
  //std::string filename1 = "/home/karthik/PR2_arm_planner/single_arm_benchmarks/benchmark_manipulation_tests/Jointspace_res/results2/";
  std::string filename1 = "/home/karthik/PR2_arm_planner/single_arm_benchmarks/benchmark_manipulation_tests/results/";
  std::string env_name = environment_name_;
  std::string filename, planner_name, eps_name;
  
  if(!strcmp(sbpl_planner_id_.c_str(), "Hstar"))
    planner_name = ompl_planner_id_;
  else
    planner_name = sbpl_planner_id_;

  std::string filename2 = filename1 + env_name + "/" + planner_name + "/";

  if(!strcmp(sbpl_planner_id_.c_str(), "WAstar") || !strcmp(ompl_planner_id_.c_str(), "RRT"))
  {
    eps_name = eps_file_;
    filename2 = filename2 + eps_name;
  }

  if(!strcmp(ompl_planner_id_.c_str(), "RRTstar") || !strcmp(ompl_planner_id_.c_str(), "BITstar"))
  {
    eps_name = timeout_file_;
    filename2 = filename2 + eps_name;
  }

  filename = filename2 + "/trajectories/";
  ...
  for(int i = 0; i < 4*5*exp_run_count_; i++)
  {
    string traj_num; 
    ostringstream convert; 
    convert << i;
    traj_num = convert.str() + ".csv";

    std::vector<std::vector<double> > way_pts;

    std::ifstream data(filename+traj_num);
    ...
    while(std::getline(data,line))
    {
        ...
        std::vector<double> single_state;
        while(std::getline(lineStream,cell,' '))
        {
          single_state.push_back(atof(cell.c_str()));    
        }
        way_pts.push_back(single_state);
    } 

    visualization_msgs::MarkerArray path_array;
    std::vector<double> coord(6,0);
    trajectory_msgs::JointTrajectory traj;
    std::vector<trajectory_msgs::JointTrajectoryPoint> vtraj;

    vtraj.resize(way_pts.size());
    
    for(size_t j = 0; j < way_pts.size(); j++)
    {
      vtraj[j].positions.resize(7);
      for(size_t k = 0; k < 7; ++k)
        vtraj[j].positions[k] = way_pts[j][k];
      
      //pathtoVisualizationMarker(path_array, coord, i%4 );
      
      //pviz_.visualizeRobot(way_pts[j], left_arm, body, 120, "state", 2, true);
      //getchar();
    }

    traj.points = vtraj;

    ROS_INFO("Visualizing Path : %d.csv", i);

    //path_marker_.publish(path_array);
    
    visualizeTrajectoryExecution(traj, start_pose_, "planned_path", 0.5);

    getchar();

    pviz_.deleteVisualizations("planned_path", 3000);

  }

}

void BenchmarkManipulationTests::pathtoVisualizationMarker(visualization_msgs::MarkerArray & path_array, std::vector<double> coord, int id)
{
...
// visualizeRobotPose(start_state, "start", 0);
  ...
  cc_->setPlanningScene(*scene);
  grid_->setReferenceFrame(scene->collision_map.header.frame_id);

  std::string env_name = name;
  env_name.erase(env_name.end()-3, env_name.end());

  if(!strcmp(env_name.c_str(), "industrial"))
  {
    rng_ = new ompl::RNG(4);
  }
  else if(!strcmp(env_name.c_str(), "countertop"))
  {     
    rng_ = new ompl::RNG(8);
  }
  else if(!strcmp(env_name.c_str(), "tabletop"))
  {       
    rng_ = new ompl::RNG(16); 
  }
  else if(!strcmp(env_name.c_str(), "tunnel"))
  {    
    rng_ = new ompl::RNG(32);
  }
  ...
  std::vector<double> goal_angles(7,0);
  for(int i = 0; i < 6; i++)
  {
    goal_pose[i] = loc_map_[exp_map_[name].goal].at(i); 
  }                                                                    

  ompl::base::StateSpacePtr space(ompl_planner_->getSPI()->getStateSpace());
  ompl::base::State *ompl_goal = space->allocState();
  std::vector<double> seed_values(7,0);  
  ROS_INFO("Looking for a valid IK solution for goal......");
  while(true)
  {
    seed_values[0] = rng_->uniformReal(-2.135398, 0.715);
    seed_values[1] = rng_->uniformReal(-0.355, 1.2963);
    seed_values[2] = rng_->uniformReal(-3.75, 0.865);
    seed_values[3] = rng_->uniformReal(-2.1213, 0.150);
    seed_values[4] = rng_->uniformReal(-M_PI, M_PI);
    seed_values[5] = rng_->uniformReal(-2.0, 0.0);
    seed_values[6] = rng_->uniformReal(-M_PI, M_PI);

    rm_->computeIK(goal_pose, seed_values, goal_angles);

    for(int i = 0 ; i < 7; i++)
    {
      ompl_goal->as<ompl::base::RealVectorStateSpace::StateType>()->values[i] = goal_angles[i];    
    }
    
    double dist;
    if(cc_->isStateValid(goal_angles, false, false, dist) && rm_->checkJointLimits(goal_angles) )
        break;
  ...
  for(int i = 0; i < 7; i++)
  {
    req2.rarm_goal[i] = goal_angles[i];
  }
  ...
  if(get_default)
  {
    for(int i = 0; i < 7; i++)
    {
      req2.rarm_start[i] = exp_map_[name].start.rangles[i];
    }
  }
  else{
    for(int i = 0; i < 7; i++)
    {
      req2.rarm_start[i] = next_start_[i];
    } 
  }
  
  //printf("SBPLStart: %f,%f,%f,%f,%f,%f,%f\n", req2.rarm_start[0], req2.rarm_start[1], req2.rarm_start[2], req2.rarm_start[3], req2.rarm_start[4], req2.rarm_start[5], req2.rarm_start[6]);


  //while(test_num <= EXP_COUNT)
  {
  ...
  // while(true)
  // {
  //   ompl_start->as<ompl::base::RealVectorStateSpace::StateType>()->values[0] = rng_->uniformReal(-2.135398, 0.715);
  //   ompl_start->as<ompl::base::RealVectorStateSpace::StateType>()->values[1] = rng_->uniformReal(-0.355, 1.2963);
  //   ompl_start->as<ompl::base::RealVectorStateSpace::StateType>()->values[2] = rng_->uniformReal(-3.75, 0.865);
  //   ompl_start->as<ompl::base::RealVectorStateSpace::StateType>()->values[3] = rng_->uniformReal(-2.1213, 0.150);
  //   ompl_start->as<ompl::base::RealVectorStateSpace::StateType>()->values[4] = rng_->uniformReal(-M_PI, M_PI);
  //   ompl_start->as<ompl::base::RealVectorStateSpace::StateType>()->values[5] = rng_->uniformReal(-2.0, 0.0);
  //   ompl_start->as<ompl::base::RealVectorStateSpace::StateType>()->values[6] = rng_->uniformReal(-M_PI, M_PI);

  //   if(ompl_planner_->getSPI()->isValid(ompl_start))
  //     break;

  // }

  // for(int i = 0; i < 7; i++)
  //   req2.rarm_start[i] = ompl_start->as<ompl::base::RealVectorStateSpace::StateType>()->values[i];

  // ompl::base::State *ompl_goal = space->allocState();

  // while(true)
  // {
  //   ompl_goal->as<ompl::base::RealVectorStateSpace::StateType>()->values[0] = rng_->uniformReal(-2.135398, 0.715);
  //   ompl_goal->as<ompl::base::RealVectorStateSpace::StateType>()->values[1] = rng_->uniformReal(-0.355, 1.2963);
  //   ompl_goal->as<ompl::base::RealVectorStateSpace::StateType>()->values[2] = rng_->uniformReal(-3.75, 0.865);
  //   ompl_goal->as<ompl::base::RealVectorStateSpace::StateType>()->values[3] = rng_->uniformReal(-2.1213, 0.150);
  //   ompl_goal->as<ompl::base::RealVectorStateSpace::StateType>()->values[4] = rng_->uniformReal(-M_PI, M_PI);
  //   ompl_goal->as<ompl::base::RealVectorStateSpace::StateType>()->values[5] = rng_->uniformReal(-2.0, 0.0);
  //   ompl_goal->as<ompl::base::RealVectorStateSpace::StateType>()->values[6] = rng_->uniformReal(-M_PI, M_PI);

  //   if(ompl_planner_->getSPI()->isValid(ompl_goal))
  //     break;

  // }

  //for(int i = 0; i < 7; i++)
  //  req2.rarm_goal[i] = ompl_goal->as<ompl::base::RealVectorStateSpace::StateType>()->values[i];
  

  std::vector<double> start_pose(6,0);
  rm_->computeFK(req2.rarm_start, rm_->getPlanningLink(), start_pose);

  visualization_msgs::Marker s_state;
  s_state.header.frame_id = "base_footprint";
  s_state.header.stamp = ros::Time();
  s_state.id = rand();
  s_state.action = visualization_msgs::Marker::ADD;
  s_state.type = visualization_msgs::Marker::CUBE;
  s_state.scale.x = 0.05;
  s_state.scale.y = 0.05;
  s_state.scale.z = 0.05;
  s_state.pose.position.x = start_pose[0];
  s_state.pose.position.y = start_pose[1];
  s_state.pose.position.z = start_pose[2];
  s_state.pose.orientation.w = 1.0;
  s_state.color.r = 1.0f;
  s_state.color.g = 1.0f;
  s_state.color.b = 0.0f;
  s_state.color.a = 1.0;
  s_state.lifetime = ros::Duration(2);
  sg_.markers.push_back(s_state);

  visualization_msgs::Marker goal_state;
  goal_state.header.frame_id = "base_footprint";
  goal_state.header.stamp = ros::Time();
  goal_state.id = rand();
  goal_state.action = visualization_msgs::Marker::ADD;
  goal_state.type = visualization_msgs::Marker::CUBE;
  goal_state.scale.x = 0.05;
  goal_state.scale.y = 0.05;
  goal_state.scale.z = 0.05;
  goal_state.pose.position.x = goal_pose[0];
  goal_state.pose.position.y = goal_pose[1];
  goal_state.pose.position.z = goal_pose[2];
  goal_state.pose.orientation.w = 1.0;
  goal_state.color.r = 1.0f;
  goal_state.color.g = 1.0f;
  goal_state.color.b = 0.0f;
  goal_state.color.a = 1.0;
  goal_state.lifetime = ros::Duration(2);
  sg_.markers.push_back(goal_state);

  sg_pub_.publish(sg_);

  //getchar();
  //sbpl plan
  printf("**********************************************SBPL starting to plan**********************************************\n");
  printf("***************************************************Test num %d***************************************************\n", test_num);

  double simplification_time = 0;
  std::vector<trajectory_msgs::JointTrajectoryPoint> sbpl_straj;
  if(!sbpl_planner_->solve(scene, req, res, sbpl_straj, req2.rarm_start, req2.rarm_goal, simplification_time))
  {
  ...
  //if(!sbpl_success && res.planning_time.toSec() < 20)
  //  continue;
  ...
  { 
    ...
    //printf("OMPLStart: %f,%f,%f,%f,%f,%f,%f\n", req2.rarm_start[0], req2.rarm_start[1], req2.rarm_start[2], req2.rarm_start[3], req2.rarm_start[4], req2.rarm_start[5], req2.rarm_start[6]);
    printf("**********************************************OMPL starting to plan**********************************************\n");
    printf("***************************************************Test num %d***************************************************\n", test_num);
    ...
    double ompl_timelim = 20.0;
    if(strcmp(timeout_file_.c_str(), "H_timeout") == 0)
        ompl_timelim = res.planning_time.toSec();
    
    if(!ompl_planner_->plan(ompl_timelim, ompl_ptraj, ompl_straj, ompl_times))
    {
      ROS_ERROR("[exp] OMPL failed to plan.");
      auto end = std::chrono::high_resolution_clock::now();
      auto duration = std::chrono::duration_cast<std::chrono::nanoseconds>(end-begin).count();
      writeStatsOMPL(1000000);
      ompl_success = false;
    }
    else
    {
      ...
      auto end = std::chrono::high_resolution_clock::now();
      auto duration = std::chrono::duration_cast<std::chrono::nanoseconds>(end-begin).count();
      writeStatsOMPL(double(duration)/1000000);
      ...
      //printf("OMPL took %f time to plan\n", ompl_times[0]);
  ...
  std::vector<trajectory_msgs::JointTrajectory> trajs(4);
  std::vector<double> times(4);
  std::vector<std::string> planners(4);
  std::vector<std::string> planner_ids(4);
  std::vector<std::string> descriptions(4);
  std::map<std::string, std::vector<double> > stats;
  trajs[0] = res.trajectory.joint_trajectory;
  trajs[1].points = sbpl_straj;
  trajs[2].points = ompl_ptraj;
  trajs[3].points = ompl_straj;
  planners[0] = "sbpl"; planners[1] = "sbpl_simplified"; planners[2] = "ompl"; planners[3] = "ompl_simplified";
  planner_ids[0] = sbpl_planner_id_; planner_ids[1] = sbpl_planner_id_; planner_ids[2] = ompl_planner_id_; planner_ids[3] = ompl_planner_id_;
  descriptions[0] = "plan"; descriptions[1] = "simplified"; descriptions[2] = "plan"; descriptions[3] = "simplified";
  times[0] = res.planning_time.toSec(); times[1] = times[0] + simplification_time; times[2] = ompl_times[0]; times[3] = times[2] + ompl_times[1]; 
  ...
  // write stats to file
  ROS_INFO("[exp] Recording stats to benchmark_stats.csv");
...
void BenchmarkManipulationTests::writeStatsOMPL(double time)
{
    static bool start_new_file = true;
    FILE* fout;
    if(start_new_file){
      fout = fopen("./ompl_stats.csv","w");
      fprintf(fout, "%% ");
     
        fprintf(fout, "%s ", "total plan time");
      fprintf(fout, "\n");
    }
    else
      fout = fopen("./ompl_stats.csv","a");

    fprintf(fout, "%.2f ", time);
    fprintf(fout, "\n");
...
bool BenchmarkManipulationTests::performAllExperiments()
{
  ...
  for (int count = 0; count < exp_run_count_; count ++)
  {
...
void BenchmarkManipulationTests::fillSingleArmPlanningRequest(RobotPose &start_state, std::string name, arm_navigation_msgs::MotionPlanRequest &req)
{
  ...
  req.group_name = group_name_;
  req.num_planning_attempts = 1;
  req.allowed_planning_time.fromSec(20.0);
  req.planner_id = ompl_planner_id_;
...
bool BenchmarkManipulationTests::printPathToFile(FILE** file, const trajectory_msgs::JointTrajectory &traj)
{
  ...
  for(size_t i = 0; i < traj.points.size(); ++i)
  {
    ...
    rm_->computeFK(jnt_pos, rm_->getPlanningLink(), p);
    ...
    if(experiment_type_ == 1)
      fprintf(*file, "%1.4f %1.4f %1.4f %1.4f %1.4f %1.4f %1.4f %1.4f %1.4f %1.4f %1.4f %1.4f %1.4f\n", traj.points[i].positions[0],traj.points[i].positions[1],traj.points[i].positions[2],traj.points[i].positions[3],traj.points[i].positions[4],traj.points[i].positions[5],traj.points[i].positions[6],p[0], p[1], p[2], p[3], p[4], p[5]);
    else
...
bool BenchmarkManipulationTests::createTrajectoryFolder(std::string exp_name)
{
  ...
  //std::string folder = benchmark_results_folder_ + experiment_group_name_ + "/trajectories";
  std::string filename1 = "/home/karthik/PR2_arm_planner/single_arm_benchmarks/benchmark_manipulation_tests/results/";
  std::string env_name = exp_name;
  env_name.erase(env_name.end()-3, env_name.end());
  std::string filename_stats = "/benchmark_stats.csv";
  std::string filename, planner_name, eps_name;
  ...
  if(!leatherman::createFolder(filename))
  {
    ROS_WARN("[exp] Failed to create the trajectory folder: %s. Maybe it exists already?", filename.c_str());
    return false;
  }
  trajectory_files_path_ = filename;
  return true;
}
...
bool BenchmarkManipulationTests::writeTrajectoriesToFile(const std::vector<trajectory_msgs::JointTrajectory> &trajs, std::vector<std::string> &planners, std::vector<std::string> &planner_ids, std::vector<std::string> &descriptions, std::string exp_name)
{
  ...
  sbpl_planner_->visualizeOMPLpath(trajs[2]);
...
bool BenchmarkManipulationTests::writeStatsToFile(const std::vector<trajectory_msgs::JointTrajectory> &trajs, std::vector<std::string> &planners, std::vector<std::string> &planner_ids, std::vector<std::string> &descriptions, std::map<std::string, std::vector<double> > &stats, std::string exp_name)
{
  ...
  filename = filename2 + filename_stats;
...
bool BenchmarkManipulationTests::initPlanners()
{
  ...
  sbpl_planner_ = new sbpl_arm_planner::SBPLArmPlannerInterface(ompl_planner_->getSPI(), rm_, cc_, as_, df_);
