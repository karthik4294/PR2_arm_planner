...
#include<egraphs/mh_egraph_planner.h>
#include<sbpl/headers.h>

class myEnv : public EnvironmentNAVXYTHETALAT, public EGraphable<vector<double> >, public EGraphDiscretize{
    ...
    void projectToHeuristicSpace(const vector<double>& coord, vector<double>& heur_coord) const{

    };

    void projectGoalToHeuristicSpace(vector<double>& heur_coord) const{
...
int main(int argc, char** argv){
  ...
  EGraphEuclideanHeuristic heur(env,1);

  EGraphManager<vector<double> > egraph_mgr(&eg, &env, &heur);
  MHEGraphPlanner<vector<double> > aegplanner(&env,3,&egraph_mgr);
