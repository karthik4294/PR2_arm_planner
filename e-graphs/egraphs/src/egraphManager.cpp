...
template <typename HeuristicType>
void EGraphManager<HeuristicType>::setNumHeuristics(int num){
  numHeuristics_ = num;
  mha_egraphs_ = EGraphDecomposer::decompose(*egraph_, num, *egraph_env_);
  mha_egraph_heurs_.resize(num);
  for(int i=0; i<num; i++){
    ROS_INFO("constructing...");
    mha_egraph_heurs_[i] = new EGraphEuclideanHeuristic(*dynamic_cast<EGraphEuclideanHeuristic*>(egraph_heur_));
    //mha_egraph_heurs_[i] = new EGraph3dGridHeuristic(*dynamic_cast<EGraph3dGridHeuristic*>(egraph_heur_));
    mha_egraph_heurs_[i]->initialize(mha_egraphs_[i]);
    mha_egraph_heurs_[i]->runPrecomputations();
    ROS_INFO("done...");
  }
}

template <typename HeuristicType>
int EGraphManager<HeuristicType>::getHeuristic(int q_id, int state_id){
    ...
    int heur_val = mha_egraph_heurs_[q_id]->getHeuristic(heur_coord);
    //boost::unique_lock<boost::mutex> time_lock(time_mutex_);
...
bool EGraphManager<HeuristicType>::setGoal(){
    ...
    stats_.num_snaps = 0;
    //clock_t time = clock();
    dynamic_cast<EGraphEuclideanHeuristic*>(egraph_heur_)->just_shortcuts = true;
    egraph_heur_->setGoal(coord);
    //printf("\n\n\nset goal in main egraph\n\n\n");
    for(unsigned int i=0; i<mha_egraph_heurs_.size(); i++){
      mha_egraph_heurs_[i]->setGoal(coord);
      //printf("\n\n\nset goal in egraph %d\n\n\n",i);
    }
...
void EGraphManager<HeuristicType>::feedbackLastPath(){
    if(numHeuristics_ != 0){
      ROS_WARN("feedback paths to E-Graph when there are multiple heuristics is unsupported!");
      assert(false);
    }
    double t0 = ros::Time::now().toSec();
