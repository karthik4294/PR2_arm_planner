...
bool EGraph::addPathHelper(const vector<vector<double> >& coords, const vector<int>& costs){
  ...
  for(unsigned int i=0; i<disc_coords.size(); i++){
    ...
    if(!v){
      ...
      if(start_new_path){
        paths.push_back(vector<vector<double> >());
        path_costs.push_back(vector<int>());
      }
      else
        path_costs.back().push_back(costs[i-1]);
      start_new_path = false;
      paths.back().push_back(coords[i]);
    }
    else
      start_new_path = true;
...
bool EGraph::savePaths(string filename){
  FILE* fout = fopen(filename.c_str(),"w");
  if(!fout){
    ROS_ERROR("Could not open file \"%s\" to save E-Graph paths",filename.c_str());
    fclose(fout);
    return false;
  }

  fprintf(fout,"%lu\n",paths.size());
  for(unsigned int i=0; i<paths.size(); i++){//for each path
    fprintf(fout,"%lu\n",paths[i].size());
    for(unsigned int j=0; j<paths[i].size(); j++){//for each waypoint
      for(unsigned int k=0; k<paths[i][j].size(); k++)//for each element
        fprintf(fout,"%f ",paths[i][j][k]);
      fprintf(fout,"\n");
    }
    for(unsigned int j=0; j<path_costs[i].size(); j++)
      fprintf(fout,"%d\n", path_costs[i][j]);
  }
  fclose(fout);
  return true;
}
bool EGraph::loadPaths(string filename){
  FILE* fin = fopen(filename.c_str(),"r");
  if(!fin){
    ROS_ERROR("Could not open file \"%s\" to load E-Graph paths",filename.c_str());
    fclose(fin);
    return false;
  }
  ...
  for(int i=0; i<numPaths; i++){
    ...
    for(int j=0; j<numPoints; j++){
      vector<double> pt(num_dims_+num_constants_,0);
      for(int k=0; k<num_dims_+num_constants_; k++)
        fscanf(fin,"%lf",&(pt[k]));
      path.push_back(pt);
    }
    paths.push_back(path);
    vector<int> costs(numPoints-1);
    for(int j=0; j<numPoints-1; j++)
      fscanf(fin,"%d",&(costs[j]));
    path_costs.push_back(costs);
  }
  fclose(fin);
  return true;
}
