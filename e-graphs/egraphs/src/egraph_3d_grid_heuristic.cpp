...
EGraph3dGridHeuristic::EGraph3dGridHeuristic(const EGraph3dGridHeuristic& other) : env_(other.env_) {
  iteration_ = 0;
  sizex_ = other.sizex_;
  sizey_ = other.sizey_;
  sizez_ = other.sizez_;
  ...
  planeSize_ = other.planeSize_;
  gridSize_ = other.gridSize_;
  cost_1_move_ = other.cost_1_move_;
  inflated_cost_1_move_ = other.inflated_cost_1_move_;

  heur = other.heur;
  ROS_INFO("other size=%d this size=%d grid_size=%d",other.heur.size(),heur.size(),gridSize_);
  sc = other.sc;
  assert(heur.size() == gridSize_);
  assert(sc.size()== gridSize_);
  ROS_INFO("copy constructor here!");
}
...
int EGraph3dGridHeuristic::getHeuristic(const vector<int>& coord){
  ...
  while(!heap.emptyheap()){
    ...
    if(cell->closed_iteration == iteration_){
      /*
      if(first_call_){
        if(count > 60000)
          first_call_ = false;
