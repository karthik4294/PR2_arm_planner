...
class EGraphManager {
    public:
        ...
        void setNumHeuristics(int num);

        void setEpsE(double epsE){
          egraph_heur_->setEpsE(epsE);
          for(unsigned int i=0; i<mha_egraph_heurs_.size(); i++)
            mha_egraph_heurs_[i]->setEpsE(epsE);
        };
        bool setGoal();
        int getHeuristic(int state_id);
        int getHeuristic(int q_id, int state_id);
        ...
        std::vector<EGraph*> getEGraphsMHA(){return mha_egraphs_;};
        ...
        std::vector<EGraphHeuristicPtr> mha_egraph_heurs_;
    private:
        ...
        std::vector<EGraph*> mha_egraphs_;
        int numHeuristics_;
